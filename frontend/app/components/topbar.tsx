...
export default function TopBar({ triggerSubscriptionModal, triggerPaymentModal, onToggleQueue, disabledToggle = false }: TopBarProps) {
    ...
    return (
        <div className="absolute z-10 w-full flex justify-between items-center font-semibold p-2 lg:max-w-xl lg:left-1/2 lg:transform lg:-translate-x-1/2">
            <Logo />
            <div className={`grid grid-cols-2 border border-slate-200 rounded-full shadow w-56 bg-black/50 text-slate-200-500 ${disabledToggle ? 'opacity-50' : ''}`}>
